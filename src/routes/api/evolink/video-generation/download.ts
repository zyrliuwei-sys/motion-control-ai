...
import { getVideoGenerationDownloadUrl } from '@/modules/evolink/service';
...
async function GET({ request }: { request: Request }) {
  try {
    ...
    const sourceUrl = await getVideoGenerationDownloadUrl({
    ...
    return new Response(upstream.body, {
      headers: {
        ...
        'Content-Disposition': `attachment; filename="ai-video-${fileId || 'video'}-${index + 1}.mp4"`,
...
export const Route = createFileRoute('/api/evolink/video-generation/download')({
  server: {
    handlers: { GET },
  },
});
